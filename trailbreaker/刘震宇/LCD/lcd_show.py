...
class DISPLAY:
  ...
  def cloth(self,x,y,color):
    for i in range(0,-1,-1):#1个字符，每个字符内有32个8位二进制数，前16个是左半边的显示，后16个是右半边的显示
      font=Cloth[i]#选择中文字符
      for j in range(16,32):#每个字符16行,先取右半边数值
        font_dat = font[j]#右半边选择一行
        for j in range(8):#开始显示
  ...
  def stone(self,x,y,color):
    for i in range(1,-1,-1):#2个字符，每个字符内有32个8位二进制数，前16个是左半边的显示，后16个是右半边的显示
      font=Stone[i]#选择中文字符
      for j in range(16,32):#每个字符16行,先取右半边数值
        font_dat = font[j]#右半边选择一行
        for j in range(8):#开始显示
